...
// Search ---
function searchHandler(store) { //MUH
    ...
    console.log("logA", store.filtered.length, formattedSearchTerm);
}
// Selection ---
...
export function dataTableSelectAll(event, store) { //MUH
...
// Pagination ---
function paginationHandler(store) { //MUH
    // store.sort = ''; // reset
    if (store.pagination) {
        // Set Current Size
        store.pagination.size = store.filtered.length;
        if (store.pagination.offset * store.pagination.limit > store.pagination.size) {
